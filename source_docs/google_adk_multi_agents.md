---
platform: Google
label: Google ADK Multi-Agent Systems
source_url: https://adk.dev/agents/multi-agents/
last_fetched: 2026-05-13
fetch_status: success
notes: REFRESHED 2026-05-13 via fetch_doc.py — body grew from 69271B to 82878B (+20%) due to ADK adding multi-language code samples upstream. || PRIOR: WAVE-2 ADD (2026-05-12). Skill A (orchestrator) grounding. Suspected summarization detected: returned content reads as paragraphs of meta-prose ("seven established patterns", "The documentation outlines...") rather than raw doc content. Placeholder summary preserves the key concepts identified (SequentialAgent / ParallelAgent / LoopAgent; sub_agents tree; transfer_to_agent; AgentTool wrapping; 7 patterns) so the file isn't useless while awaiting manual fetch.
size_bytes: 82878
---
...
ADK includes specialized agents derived from `BaseAgent` that don't perform tasks themselves but orchestrate the execution flow of their `sub_agents`.
  ...
  * **[`SequentialAgent`](<../workflow-agents/sequential-agents/>):** Executes its `sub_agents` one after another in the order they are listed.
    * **Context:** Passes the _same_ [`InvocationContext`](<../../runtime/>) sequentially, allowing agents to easily pass results via shared state.
