...
notes: WAVE-2 ADD (2026-05-12). Skill A (orchestrator) grounding. Key facts captured: Traces (workflow name, trace ID, group ID, metadata); Spans (start/end times, span data for agents/LLM-generations/function-calls); tracing enabled by default, auto-wraps Runner ops + Agent runs + LLM generations + tool calls + guardrails + handoffs + audio; disable via env var or RunConfig.tracing_disabled; trace() context manager + manual start/finish; contextvar-based concurrency; generation_span() and function_span() capture sensitive data — control via RunConfig.trace_include_sensitive_data or OPENAI_AGENTS_TRACE_INCLUDE_SENSITIVE_DATA env var; add_trace_processor() vs set_trace_processors(); 30+ ecosystem integrations (Weights & Biases, Datadog, LangSmith, Braintrust, others).
size_bytes: 13582
---
...
Tracing is enabled by default. You can disable it in three common ways:
  ...
  2. You can globally disable tracing in code with [`set_tracing_disabled(True)`](<../ref/#agents.set_tracing_disabled> "set_tracing_disabled")
  3. You can disable tracing for a single run by setting [`agents.run.RunConfig.tracing_disabled`](<../ref/run/#agents.run.RunConfig.tracing_disabled> "tracing_disabled
...
By default, the trace is named "Agent workflow". You can set this name if you use `trace`, or you can configure the name and other properties with the [`RunConfig`](<../ref/run/#agents.run.RunConfig> "RunConfig
...
[`flush_traces()`](<../ref/tracing/#agents.tracing.flush_traces> "flush_traces") blocks until currently buffered traces and spans are exported, so call it after `trace()` closes to avoid flushing a partially built trace. You can skip this call when the default export latency is acceptable.
...
You can use an OpenAI API key with non-OpenAI models to enable free tracing in the OpenAI Traces dashboard without needing to disable tracing. See the [Third-party adapters](<../models/#third-party-adapters>) section in the Models guide for adapter selection and setup caveats.
