---
platform: Anthropic
label: Subagents in the Claude Agent SDK
source_url: https://code.claude.com/docs/en/agent-sdk/subagents
last_fetched: 2026-05-12
fetch_status: success
fetch_error: none
notes: Programmatic AgentDefinition (description/prompt/tools/disallowedTools/model/skills/memory/mcpServers/maxTurns/background/effort/permissionMode), filesystem-based agents in .claude/agents/, built-in general-purpose subagent via Agent tool. Subagent context isolation, parallelization, tool restrictions. What subagents inherit (own system prompt + Agent tool prompt only; not parent conversation history or preloaded skills unless listed). Dynamic factory pattern. Detecting subagent invocation via tool_use blocks with name="Agent" (renamed from "Task" in v2.1.63) and parent_tool_use_id. Resuming subagents via session_id + agentId. Subagents cannot spawn subagents. Fetched via WebFetch on 2026-05-12.
---
> ## Documentation Index
