...
notes: WAVE-2 ADD (2026-05-12). Skill B (knowledge) grounding. Key facts captured: two caching modes — implicit (default for Gemini 2.5+; auto cost savings on cache hits; no developer action) and explicit (manual cache with guaranteed cost reduction; TTL default 1 hour); implicit cache minimum token thresholds 1024-4096 by model; hit-rate strategies (large+common content at beginning, similar-prefix requests in short timespan); cache_hit tokens visible in response.usage_metadata; explicit cache minimum tokens — Gemini 3 Flash Preview 1024, Gemini 3 Pro Preview 4096; explicit cache supports videos, PDFs, text files, and system instructions; use case fit — chatbots with extensive system instructions, video analysis, document queries, code repository work; billing — cached token count at reduced rates + storage duration based on TTL + standard charges for non-cached and output.
size_bytes: 19366
---
