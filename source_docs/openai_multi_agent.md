---
platform: OpenAI
label: OpenAI Agents SDK — Multi-Agent Orchestration
source_url: https://openai.github.io/openai-agents-python/multi_agent/
last_fetched: 2026-05-13
fetch_status: success
notes: WAVE-2 ADD (2026-05-12). Skill A (orchestrator) grounding. Key facts captured: two orchestration strategies (LLM-driven vs code-based); two SDK patterns (agents-as-tools via Agent.as_tool() vs handoffs via specialist becoming active agent); LLM orchestration tactics (specialized agents, evaluation loops, iterative prompt refinement); code-based patterns (structured output classification, chaining, asyncio.gather parallelism, evaluator feedback loops).
size_bytes: 4934
---
...
If you want the core SDK primitives behind this style of orchestration, start with [tools](<../tools/>), [handoffs](<../handoffs/>), and [running agents](<../running_agents/>).
...
## Related guides
  ...
  * [Agents](<../agents/>) for composition patterns and agent configuration.
  * [Tools](<../tools/#agents-as-tools>) for `Agent.as_tool()` and manager-style orchestration.
  * [Handoffs](<../handoffs/>) for delegation between specialist agents.
  * [Running agents](<../running_agents/>) for per-run orchestration controls and conversation state.
  * [Quickstart](<../quickstart/>) for a minimal end-to-end handoff example.
