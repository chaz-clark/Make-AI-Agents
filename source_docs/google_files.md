---
platform: Google
label: Gemini API — Files API
source_url: https://ai.google.dev/gemini-api/docs/files
last_fetched: 2026-05-13
fetch_status: success
notes: WAVE-2 ADD (2026-05-12). Skill B (knowledge) grounding. Key facts captured: file limits (2GB per file, 20GB per project, 48-hour retention, PDFs ≤50MB, use Files API when total request size exceeds 100MB); free across all supported regions; core operations (files.upload, files.get, files.list, files.delete); auto-deletion at 48h; multimodal prompting best practices captured (clear/concise instructions, few-shot examples, sequential decomposition, output format specification, image-before-text for single-image prompts; troubleshooting via describe-first, explicit element references, step-by-step reasoning, lower temperature).
size_bytes: 28053
---
