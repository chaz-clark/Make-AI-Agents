...
notes: WAVE-2 ADD (2026-05-12). Skill A (orchestrator) grounding. Key facts captured: three execution methods (Runner.run async, Runner.run_sync, Runner.run_streamed); agent loop (LLM call → final-output|handoff|tool-calls → loop or end; MaxTurnsExceeded on overflow); input types (string, item list, RunState); streaming via RunResultStreaming.stream_events(); RunConfig categories (model/defaults, guardrails/handoffs, observability, tool management); four persistence strategies (to_input_list, session, conversation_id, previous_response_id); call_model_input_filter; error_handlers dict for "max_turns" and "model_refusal"; tool_execution with max_function_tool_concurrency vs ModelSettings.parallel_tool_calls; reasoning_item_id_policy ("preserve"/"omit"); durable execution integrations (Dapr, Temporal, Restate, DBOS); exception hierarchy (AgentsException, MaxTurnsExceeded, ModelBehaviorError, ToolTimeoutError, UserError, InputGuardrailTripwireTriggered, OutputGuardrailTripwireTriggered).
size_bytes: 34473
---
...
When you use the run method in `Runner`, you pass in a starting agent and input. The input can be:
  ...
  * a [`RunState`](<../ref/run_state/#agents.run_state.RunState> "RunState
...
The runner then runs a loop:
  ...
  3. If we exceed the `max_turns` passed, we raise a [`MaxTurnsExceeded`](<../ref/exceptions/#agents.exceptions.MaxTurnsExceeded> "MaxTurnsExceeded") exception. Pass `max_turns=None` to disable this turn limit.
...
This is the Responses API over websocket transport, not the [Realtime API](<../realtime/guide/>).

For transport-selection rules and caveats around concrete model objects or custom providers, see [Models](<../models/#responses-websocket-transport>).
...
Use [`responses_websocket_session()`](<../ref/responses_websocket_session/#agents.responses_websocket_session>) when you want a shared websocket-capable provider and `RunConfig` across multiple runs \(including nested agent-as-tool calls that inherit the same `run_config`\).
...
##### Model, provider, and session defaults
  ...
  * [`session_settings`](<../ref/run/#agents.run.RunConfig.session_settings> "session_settings

  
      class-attribute
      instance-attribute
  "): Overrides session-level defaults \(for example, `SessionSettings(limit=...)`\) when retrieving history during a run.
  * [`session_input_callback`](<../ref/run/#agents.run.RunConfig.session_input_callback> "session_input_callback

  
      class-attribute
      instance-attribute
  "): Customize how new user input is merged with session history before each turn when using Sessions. The callback can be sync or async.
...
##### Guardrails, handoffs, and model input shaping
  ...
  * [`handoff_input_filter`](<../ref/run/#agents.run.RunConfig.handoff_input_filter> "handoff_input_filter
  ...
  "): A global input filter to apply to all handoffs, if the handoff doesn't already have one. The input filter allows you to edit the inputs that are sent to the new agent. See the documentation in [`Handoff.input_filter`](<../ref/handoffs/#agents.handoffs.Handoff.input_filter> "input_filter
      ...
      instance-attribute
  ") for more details.
  * [`nest_handoff_history`](<../ref/run/#agents.run.RunConfig.nest_handoff_history> "nest_handoff_history

  
      class-attribute
      instance-attribute
  "): Opt-in beta that collapses the prior transcript into a single assistant message before invoking the next agent. This is disabled by default while we stabilize nested handoffs; set to `True` to enable or leave `False` to pass through the raw transcript. All [Runner methods](<../ref/run/#agents.run.Runner> "Runner") automatically create a `RunConfig` when you do not pass one, so the quickstarts and examples keep the default off, and any explicit [`Handoff.input_filter`](<../ref/handoffs/#agents.handoffs.Handoff.input_filter> "input_filter

  
      class-attribute
      instance-attribute
  ") callbacks continue to override it. Individual handoffs can override this setting via [`Handoff.nest_handoff_history`](<../ref/handoffs/#agents.handoffs.Handoff.nest_handoff_history> "nest_handoff_history

  
      class-attribute
      instance-attribute
  ").
  * [`handoff_history_mapper`](<../ref/run/#agents.run.RunConfig.handoff_history_mapper> "handoff_history_mapper

  
      class-attribute
      instance-attribute
  "): Optional callable that receives the normalized transcript \(history + handoff items\) whenever you opt in to `nest_handoff_history`. It must return the exact list of input items to forward to the next agent, allowing you to replace the built-in summary without writing a full handoff filter.
  * [`call_model_input_filter`](<../ref/run/#agents.run.RunConfig.call_model_input_filter> "call_model_input_filter

  
      class-attribute
      instance-attribute
  "): Hook to edit the fully prepared model input \(instructions and input items\) immediately before the model call, e.g., to trim history or inject a system prompt.
  * [`reasoning_item_id_policy`](<../ref/run/#agents.run.RunConfig.reasoning_item_id_policy> "reasoning_item_id_policy

  
      class-attribute
      instance-attribute
  "): Control whether reasoning item IDs are preserved or omitted when the runner converts prior outputs into next-turn model input.
...
##### Tracing and observability
  ...
  * [`tracing`](<../ref/run/#agents.run.RunConfig.tracing> "tracing

  
      class-attribute
      instance-attribute
  "): Pass a [`TracingConfig`](<../ref/tracing/#agents.tracing.TracingConfig> "TracingConfig") to override trace export settings such as the per-run tracing API key.
...
This is separate from provider-side [`ModelSettings.parallel_tool_calls`](<../ref/model_settings/#agents.model_settings.ModelSettings.parallel_tool_calls> "parallel_tool_calls

  
      class-attribute
      instance-attribute
  "). `parallel_tool_calls` controls whether the model is allowed to emit multiple tool calls in a single response. `tool_execution.max_function_tool_concurrency` controls how the SDK executes local function tool calls after the model has emitted them.
...
The formatter receives [`ToolErrorFormatterArgs`](<../ref/run_config/#agents.run_config.ToolErrorFormatterArgs> "ToolErrorFormatterArgs
...
Session persistence cannot be combined with server-managed conversation settings \(`conversation_id`, `previous_response_id`, or `auto_previous_response_id`\) in the same run. Choose one approach per call.
...
If a run pauses for approval and you resume from a [`RunState`](<../ref/run_state/#agents.run_state.RunState> "RunState

  
      dataclass
  "), the SDK keeps the saved `conversation_id` / `previous_response_id` / `auto_previous_response_id` settings so the resumed turn continues in the same server-managed conversation.
...
The SDK automatically retries `conversation_locked` errors with backoff. In server-managed conversation runs, it rewinds the internal conversation-tracker input before retrying so the same prepared items can be resent cleanly.

In local session-based runs \(which cannot be combined with `conversation_id`, `previous_response_id`, or `auto_previous_response_id`\), the SDK also performs a best-effort rollback of recently persisted input items to reduce duplicate history entries after a retry.

This compatibility retry happens even if you do not configure `ModelSettings.retry`. For broader opt-in retry behavior on model requests, see [Runner-managed retries](<../models/#runner-managed-retries>).
...
The return value must be a [`ModelInputData`](<../ref/run_config/#agents.run_config.ModelInputData> "ModelInputData

  
      dataclass
  ") object. Its `input` field is required and must be a list of input items. Returning any other shape raises a `UserError`.
...
If you are using a session, `call_model_input_filter` runs after session history has already been loaded and merged with the current turn. Use [`session_input_callback`](<../ref/run/#agents.run.RunConfig.session_input_callback> "session_input_callback

  
      class-attribute
      instance-attribute
  ") when you want to customize that earlier merge step itself.
...
For tool approval pause/resume patterns, start with the dedicated [Human-in-the-loop guide](<../human_in_the_loop/>). The integrations below are for durable orchestration when runs may span long waits, retries, or process restarts.
...
The SDK raises exceptions in certain cases. The full list is in [`agents.exceptions`](<../ref/exceptions/#agents.exceptions>). As an overview:
  ...
  * [`ToolTimeoutError`](<../ref/exceptions/#agents.exceptions.ToolTimeoutError> "ToolTimeoutError"): This exception is raised when a function tool call exceeds its configured timeout and the tool uses `timeout_behavior="raise_exception"`.
