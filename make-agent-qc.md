...
**Description**: Every agent built from `make-agent.md` MUST embed the behavioral discipline correctly. The QC validates the embedding is structurally complete — not just that the words "Behavioral Discipline" appear somewhere in the file.
...
- **`make-agent.md`**: Reference template for agent creation
...
| **Complexity** | standard |
| **Key Files** | `make_agent_qc.json`, `make-agent-qc.md` |
