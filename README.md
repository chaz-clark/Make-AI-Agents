...
│   ├── behavioral-discipline.md       # Toyota Way + Karpathy framework (YAML frontmatter + narrative)
...
Full detail — the 10 principles (P-001 through P-010), when each applies, when each can be overridden — lives at [`knowledge/behavioral-discipline.md`](knowledge/behavioral-discipline.md). The `make_agent_qc` skill validates new specs against the BD-QC family (BD-QC-001 through BD-QC-009).
